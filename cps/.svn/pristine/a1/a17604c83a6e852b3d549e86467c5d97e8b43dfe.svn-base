package com.duanrong.cps.business.invest.dao.impl;
...
public class InvestDaoImpl extends BaseDaoImpl<Invest> implements InvestDao {
	...
	public InvestDaoImpl() {
		this.setMapperNameSpace("com.duanrong.cps.business.invest.model.Invest");
	}
	...
	/**
	 * 
	 * @description 根据组合条件查询所有借款记录(该方法已经移动到LoanDaoImpl中)
	 * @author 孙铮
	...
	// List<Invest> invests =
	// this.getSqlSession().selectList(this.getMapperNameSpace() +
	// ".getInvestsByGroupCondition", invest);
	// return invests;
	// }
	...
	public PageInfo<Invest> GetPagerInvestByLoanId(int pageIndex, int pageSize,
		...
		PageInfo<Invest> result = new PageInfo<Invest>();
	...
	public PageInfo<Invest> getInvestRecordsNetLoanEye(int pageNo, int pageSize, Map<String, Object> map){
		PageHelper.startPage(pageNo, pageSize);
		List<Invest> list = 
				this.getSqlSession().selectList(getMapperNameSpace()+".getInvestRecordsNetLoanEye", map);
	...
	public Date getInvestDateFirstOne(String loanId) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getInvestDateFirstOne", loanId);
	}

	@Override
	public Object getTotalCount(String loanId) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getTotalCount", loanId);
	}

	@Override
	public List<Invest> getInvestRecords(String loanId) {
		return this.getSqlSession().selectList(getMapperNameSpace()+".getInvestRecords", loanId);
	}

	@Override
	public int getLendCount(String date) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getLendCount", date);
	}
	...
	public List<Invest> getAccInvest(String userId, String startTime,
		...
		if("".equals(investStatus) || null==investStatus || "null".equals(investStatus)){
			paramMap.put("investStatus", "");
		}else if(investStatus.equals(0)){
			paramMap.put("investStatus", "0");
		}else if(investStatus.equals(1)){
			paramMap.put("investStatus", "1");
		}else if(investStatus.equals(2)){
			paramMap.put("investStatus", "2");
		}
		
		paramMap.put("offset", Integer.valueOf(offset));
		paramMap.put("limit", Integer.valueOf(limit));
		if(StringUtils.isBlank(investRecordId) || investRecordId.equals("null")){
			paramMap.put("investRecordId", "");
		}else{
			paramMap.put("investRecordId", investRecordId);
		}
		return this.getSqlSession().selectList(getMapperNameSpace()+".getAccInvest", paramMap);
	}
	...
	public List<Repay> getRepays(String id, String userId) {
		...
		return this.getSqlSession().selectList(getMapperNameSpace()+".getRepays", paramMap);
	}
	...
	public Repay getNextRepay(String id, String userId) {
		...
		paramMap.put("userId", userId);
		paramMap.put("loanId", id);
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getNextRepay", paramMap);
	}

	@Override
	public FengchelicaiRepayment getRepayMentRecordsList(
			Map<String, Object> map) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getRepayMentRecordsList", map);
	}

	@Override
	public int getAmountMoney(String loanId) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getAmountMoney", loanId);
	}

	@Override
	public Double getTotalMoney4XiCai(String userId) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getTotalMoney4XiCai", userId);
	}
	...
	public List<Invest> getInvestInfo(String startdate, String enddate,
		...
		System.out.println("#############"+Integer.valueOf(pagesize));
		List<Invest> result = this.getSqlSession().selectList(getMapperNameSpace()+".getInvestInfo", paramMap);
	...
	public Double getTotalMoney4AlreadyFundraising(String id) {
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getTotalMoney4AlreadyFundraising", id);
	}
	...
	public int getTotalInvest(String startdate, String enddate) {
		...
		paramMap.put("startdate", startdate);
		paramMap.put("enddate", enddate);
		System.out.println("################返回total");
		return this.getSqlSession().selectOne(getMapperNameSpace()+".getTotalInvest", paramMap);
	}
