...
<body>
	...
	<div id="content">
		...
		<div class="m-right">
			<div class="right-nav">
				<ul>
					<li>当前位置：比搜益推送投资的记录</li>
				</ul>
				<br/>
				<form action="/bsy/bsyPushInvest" method="get">
					...
					<div id="rule"></div>
						<div class="box-table">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<c:forEach items="${pageInfo.results}" var="pushInvest" varStatus="Num">
								<tr>
								<td>${pushInvest.loanId}
								<br>${pushInvest.loanName}
								</td>
								<td>${pushInvest.investId}</td>
								<td>${pushInvest.userId}</td>
								<td>${pushInvest.investMoney}</td>
								<td>${pushInvest.period}</td>
								<td>
									<fmt:formatNumber type="number" value="${pushInvest.rate}" pattern="0.00" maxFractionDigits="2"/>  
								</td>
								<td>${pushInvest.profitWay}</td>
								<td>
								<fmt:formatDate value="${pushInvest.interestDate}" type="both"/>
								</br>
								<fmt:formatDate value="${pushInvest.repayDate}" type="both"/>	
								</td>
								<td>${pushInvest.orderStatus}</td>
								<td>${pushInvest.loanStatus}</td>
								<td>${pushInvest.investStatus}</td>
								<td>
								<fmt:formatDate value="${pushInvest.nowTime}" type="both"/>
								</td>
								</tr>
								</c:forEach>
								<tr></tr>
