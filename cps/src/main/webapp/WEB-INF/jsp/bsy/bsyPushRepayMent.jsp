...
<body>
	...
	<div id="content">
		...
		<div class="m-right">
			<div class="right-nav">
				<ul>
					<li>当前位置：比搜益推送回款记录</li>
				</ul>
				<br/>
				<form action="/bsy/bsyPushRepayMent" method="get">
					...
					<div id="rule"></div>
						<div class="box-table">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<c:forEach items="${pageInfo.results}" var="bsyPushRepayMent" varStatus="Num">
								<tr>
								<td>${bsyPushRepayMent.loanId}</td>
								<td>${bsyPushRepayMent.investId}</td>
								<td>${bsyPushRepayMent.userId}</td>
								<td>
								<fmt:formatDate value="${bsyPushRepayMent.repayDate}" type="both"/>
								</td>
								<td>${bsyPushRepayMent.repayMoney}</td>
								<td>${bsyPushRepayMent.loanStatus}</td>
								<td>
								<fmt:formatDate value="${bsyPushRepayMent.nowTime}" type="both"/>
								</td>
								<td>${bsyPushRepayMent.repayId}</td>
								</tr>
								</c:forEach>
								<tr></tr>
							</table>							
