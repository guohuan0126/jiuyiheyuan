...
import com.duanrong.business.service.impl.PushServiceImpl;
...
public class P2PEyeHttpUtil {
	...
	private static Logger logger = Logger.getLogger(P2PEyeHttpUtil.class);
	...
	public static int sendPost(JSONObject param,String mod){
		...
		logger.info("推送前数据："+param);
		String data = paramFormat(param);
		logger.info("拼接好的参数："+data);
		String signKey = "";
		try {
			signKey = MD5.getMd5(data.getBytes("UTF-8")); //MD5
			data = URLEncoder.encode(TripleDesUtil.encrypt(data,key), "UTF-8");
			logger.info("线上加密："+data);
      		logger.info("线上解密："+TripleDesUtil.decrypt(URLDecoder.decode(data,"UTF-8"),key));
		} catch (Exception e) {
			e.printStackTrace();
		}
		json.put("data",data);
		...
		try {
			...
			logger.info(httpUrl+"请求耗时"+time+"毫秒"+",请求IP:"+ip);
