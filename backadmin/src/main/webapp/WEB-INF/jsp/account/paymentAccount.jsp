	...
	<section>
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<i class="fa fa-table"></i>当前位置：支付账户管理
				</h2>
					...
					<tbody id="treet1">
						<c:forEach items="${list}" var="item">
							<tr>
								<%-- <td>${item.id }</td> --%>
								<td><c:choose>
										<c:when test="${item.channel eq 'Baofoo'}">
	                           		 宝付
	                           </c:when>
										<c:when test="${item.channel eq 'Fuiou'}">
	                       		            富友
	                           </c:when>
										<c:when test="${item.channel eq 'JDpay'}">
		                       		 京东
		                        </c:when>
										<c:when test="${item.channel eq 'Yeepay'}">
		                       		易宝
		                        </c:when>
										<c:otherwise>
	                            	${item.channel}
...
<%-- 								<td>${item.balance }</td>
								...
								<td><a style="cursor:pointer"
									href="/paymentAccount/paymentAccountIn?channel=${item.channel}">线下充值</a>|
									<a style="cursor:pointer"
									href="/paymentAccount/paymentAccountOut?channel=${item.channel}">线下提现</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<div class="pageheader">
					<h2>
						<i class="fa fa-table"></i>当前位置：支付账户流水管理
					</h2>

				</div>
				<div class="panel-heading">
					<form class="form-inline" action="/account/paymentAccountList"
						method="post">
