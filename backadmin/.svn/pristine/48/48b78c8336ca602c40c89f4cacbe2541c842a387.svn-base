...
public class ItemFlowServiceImpl implements ItemFlowService {
	...
	public boolean oprate(ItemFlow itemFlow, String userId) {
		...
		ItemFlow item = readCurrentItemFlow(
	...
	public boolean issueItem(int flowId, int itemId, ItemType type) {
		int flag = 0;
		Flow flow = flowService.readFlowByFlowId(flowId);
	...
	@Override
	public PageInfo<ItemFlow> readAllItemFlow(int pageNo, int pageSize,
			ItemFlow itemFlow) {
	...
	@Override
	public ItemFlow readCurrentItemFlow(int itemId, ItemType type, String userId) {
		List<ItemFlow> itemFlows = readItemFlowByItemID(itemId, type);
		...
		if (!itemFlows.isEmpty()) {						
			...
			if(itemFlow != null){
				if(readItemFlowByItemID(itemFlow.getId(), type, userId).size() == 0 ){
					return null;
				}
			}
		}
	...
	@Override
	public List<ItemFlow> readItemFlowByItemID(int itemId, ItemType type) {
	...
	@Override
	public List<ItemFlow> readItemFlowByItemID(int id, ItemType type,
			String userId) {
		Map<String, Object> map = new HashMap<>();
		map.put("id", id);
		map.put("itemType", type);
