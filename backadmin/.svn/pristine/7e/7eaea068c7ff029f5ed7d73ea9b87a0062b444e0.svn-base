...
public class DateUtil {
	...
	public static Date addDay(int dayAmount) {
		return addInteger(new Date(), Calendar.DATE, dayAmount);
	}
	...
	public static Date DayAdd(Date d, int dayAmount) {
		Calendar cNow = Calendar.getInstance();
		cNow.setTime(d);
		cNow.add(Calendar.DATE, dayAmount);// 日期增加。
		return cNow.getTime();
	}
	...
	public static int DayDifference(Date date1, Date date2)
			throws ParseException {		
		int betweenDate = (int) ((date2.getTime() - date1.getTime()) / (24 * 60 * 60 * 1000));
		return betweenDate;
	}
	 ...
	 * @return
	 */
	public static Date DifferenceDay(Date date, int days) {
		SimpleDateFormat dft = new SimpleDateFormat("yyyy-MM-dd");
		Calendar date1 = Calendar.getInstance();
		date1.setTime(date);
		date1.set(Calendar.DATE, date1.get(Calendar.DATE) - days);
		try {
			return dft.parse(dft.format(date1.getTime()));
		} catch (ParseException e) {
	 ...
	 * @return
	 */
	public static String DifferenceDay1(Date date, int days) {
		return new SimpleDateFormat("yyyy-MM-dd hh:mm:ss")
				.format(DifferenceDay(date, days));
	}
	 ...
	 * @return
	 */
	public static int mothsToDays(Date date, int moths) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.add(Calendar.MONTH, moths);
		int betweenDate = (int) ((cal.getTime().getTime() - date.getTime()) / (24 * 60 * 60 * 1000));
		return betweenDate;
	}
