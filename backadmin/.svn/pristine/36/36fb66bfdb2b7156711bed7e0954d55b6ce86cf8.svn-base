...
import com.duanrong.business.msg.SimpleEmailSend;
...
public class EmailServiceImpl implements EmailService {
	...
	@Override
	public PageInfo<Email> readPageInfo(int pageNo, int pageSize, Map map) {
	...
	private String readEmailConfigById(String id) {		
		return emailDao.getConfigById(id);
	}

	@Override
	public String sendEmail(String loanId, String userId) {
		String mailSmtp = readEmailConfigById("mail_smtp");
		String mailUserName = readEmailConfigById("mail_username");
		String mailPassword = readEmailConfigById("mail_password");
		String mailPersonal = readEmailConfigById("mail_personal");
		if(MyStringUtils.isNotAnyBlank(loanId, mailSmtp, mailUserName, mailPassword)){
			List<Email> emails = readSendEmailByLoan(loanId);
			int i = 0, j = 0;
