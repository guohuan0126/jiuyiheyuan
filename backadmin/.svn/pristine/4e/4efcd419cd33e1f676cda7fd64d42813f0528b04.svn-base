...
public class DemandtreasureServiceImpl implements DemandtreasureService {
	...
	public int insertDayDate(Date now) {
		List<String> userIds = demandTreasureBillService.readDemandTreasureUser();
	...
	private double getInterest(String userId, Date date) {
		return ArithUtil.round(
				demandTreasureBillService.readDemandTreasureValidMoney(userId, date)
						* dateRateService.readRate(new Date()) / 365, 2);
	}

	@Override
	public double readDemandInterest(String start, String end) {
		
		return demandtreasureDao.readDemandInterest(start,end);
	}
