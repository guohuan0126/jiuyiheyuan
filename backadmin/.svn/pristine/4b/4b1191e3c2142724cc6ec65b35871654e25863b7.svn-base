	...
	<section>
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<i class="fa fa-table"></i>当前位置：放款记录
				</h2>
			</div>
							...
							<thead>
								...
								<tr>
								   <td>${paymentLoanRecords.loandataId}</td>
									...
									<td>${base:decrypt(paymentLoanRecords.mobileNumber)}</td>	
									<td>${base:decrypt(paymentLoanRecords.bankCard)}</td>
									<td>${paymentLoanRecords.loanId}</td>	
									<td>${paymentLoanRecords.loanName}</td>	
									<td>
									<fmt:formatDate value="${paymentLoanRecords.requestTime}" type="both"/>
									</td>	
									<td>${paymentLoanRecords.money}</td>	
									<td>${paymentLoanRecords.paymentChannel}</td>	
									<td>${paymentLoanRecords.serialPay}</td>	
									<td>${paymentLoanRecords.msg}</td>	
									<td>
										<c:if test="${paymentLoanRecords.status==1}">成功</c:if>
										<c:if test="${paymentLoanRecords.status==0}">成功</c:if>
									</td>	
								</tr>
							</c:forEach>	
						</table>
