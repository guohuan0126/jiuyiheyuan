...
public class SqlServer {
	...
	// 行号列
	private static final Column PAGE_ROW_NUMBER_COLUMN = new Column(
			PAGE_ROW_NUMBER);
	// TOP 100 PERCENT
	...
	private SelectBody wrapSetOperationList(SetOperationList setOperationList) {
		// 获取最后一个plainSelect
		PlainSelect plainSelect = setOperationList.getPlainSelects().get(
				setOperationList.getPlainSelects().size() - 1);
		...
		for (SelectItem selectItem : plainSelect.getSelectItems()) {
			...
			if (selectItem instanceof SelectExpressionItem) {
				...
				if (selectExpressionItem.getAlias() != null) {
					// 直接使用别名
					Column column = new Column(selectExpressionItem.getAlias()
							.getName());
					...
					Column column = (Column) selectExpressionItem
							.getExpression();
	...
	private void addRowNumber(PlainSelect plainSelect, String orderBy) {
		...
		plainSelect.getSelectItems().add(0,
				new SelectExpressionItem(orderByColumn));
	}
