 ...
 * @since 3.6.0 项目地址 : http://git.oschina.net/free/Mybatis_PageHelper
 */
...
public class SqlUtil {
	...
	public SqlUtil(String strDialect) {
		...
		try {
			...
			throw new IllegalArgumentException("Mybatis分页插件dialect参数值错误，可选值为["
					+ dialects + "]");
		}
	}
	...
	public void processPageMappedStatement(MappedStatement ms,
			SqlSource sqlSource, Page page, Object[] args) {
	...
	private static class OracleParser extends SimpleParser {
		...
		public String getPageSql(String sql) {
			...
			sqlBuilder
					.append("select * from ( select tmp_page.*, rownum row_id from ( ");
			sqlBuilder.append(sql);
			sqlBuilder
					.append(" ) tmp_page where rownum <= ? ) where row_id > ?");
			return sqlBuilder.toString();
		}
	...
	private static class SqlServerParser extends SimpleParser {
		...
		public String getPageSql(String sql) {
			Page page = getLocalPage();
			return pageSql.convertToPageSql(sql, page.getStartRow(),
