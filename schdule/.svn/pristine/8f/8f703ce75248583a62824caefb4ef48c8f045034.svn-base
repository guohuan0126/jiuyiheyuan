...
public class TrusteeshipOperationDaoImpl extends
	...
	@Override
	public List<TrusteeshipOperation> getTO4automaticQuery(
			TrusteeshipOperation trusteeshipOperation) {
		return this.getSqlSession().selectList(
				this.getMapperNameSpace() + ".getTO4automaticQuery",
				trusteeshipOperation);
	}
	
	@Override
	public List<TrusteeshipOperation> getTOautomaticQuery(
			Date date) {
		return this.getSqlSession().selectList(
				this.getMapperNameSpace() + ".getTOautomaticQuery",
				date);
	}
