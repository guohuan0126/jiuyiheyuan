...
public class DateRateServiceImpl implements DateRateService {
	...
	public PageInfo<DateRate> findPageInfo(int pageNo, int pageSize, Map map) {
		return dateRateDao.pageInfo(pageNo, pageSize, map);
	}
	...
	public void addRate(String year, String month,double rate) {
		...
		for(int i=1;i<=max;i++){
			...
			Date date=c.getTime();
			List<DateRate> list=dateRateDao.getRateList(date);
			for(DateRate r:list){
	...
	public double getRate(Date date) {
		return dateRateDao.getRate(date);
	}

	@Override
	public List<DateRate> getRateList(Date date) {
		return dateRateDao.getRateList(date);
	}
