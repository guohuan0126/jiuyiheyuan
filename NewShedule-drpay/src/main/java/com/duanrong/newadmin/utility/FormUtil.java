...
public class FormUtil {
		...
		for(int i = 0; i < fields.length; i++){
           try {
        	   if(!fields[i].getName().equals("serialVersionUID")){
	               ...
	               if(MyStringUtils.isNotAnyBlank(pd.getReadMethod().invoke(obj))){
           ...
           try {
        	   if(!fields[i].getName().equals("serialVersionUID") && fields[i].getType() == String.class){
	               ...
	               if(MyStringUtils.isNotAnyBlank(param)){
	            	   Object o = null;
	            	   if(EncodingUtil.getEncoding(param).equals("ISO8859-1")){
	            		   o =  new String(param.getBytes("ISO8859-1"), "UTF-8");
	            	   }else{
	...
	public static Map<String, Object> getForParamToBean(Map<String, Object> map) throws UnsupportedEncodingException{
		...
		while (it.hasNext()) {
			Map.Entry<String, Object> entry = it.next();
			if(MyStringUtils.isNotAnyBlank(entry.getKey()) && MyBeanUtils.nonNull(entry.getValue())){
				if(entry.getValue() instanceof String && MyBeanUtils.nonNull(entry.getValue())){
					 if(EncodingUtil.getEncoding(entry.getValue().toString()).equals("ISO8859-1")){
						 obj.put(entry.getKey(), new String(entry.getValue().toString().getBytes("ISO8859-1"), "UTF-8"));	
	            	   }else{
