		...
		try {
			if (!forks.isEmpty()) {
				...
				if (type.equals("in")) {
					...
					map.put("id", id);
					map.put("money", money);
					DemandTreasureInvest invest = demandtreasureInvestDao.get(id);
					...
					opration.setDemandLoanId(invest.getDemandLoanId());
					opration.setDemandTransferId(invest.getDemandTransferId());
					opration.setDemandTreasureInvestId(id);
					opration.setMoney(invest.getMoney());
					opration.setType("out");
					opration.setRemark("天天赚到期续投转出");
