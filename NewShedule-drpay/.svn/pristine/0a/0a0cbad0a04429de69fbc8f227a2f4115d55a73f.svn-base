		...
		if ("/".equals(url)) {
			...
			if (MyStringUtils.isNotAnyBlank(param) && param.contains("=")) {
				...
				if (request.getSession().getAttribute("super") == null) {
					System.out.println(permissionService.getSuper(method));
					if (permissionService.getSuper(method).endsWith(value)) {
			...
			/*if(getIpAddr(request).contains("192.168.1")){ return permiss(request, response, url); }	*/
						
			// 授权IP可以直接访问
			List<String> ips = new ArrayList<>();
			if (request.getSession().getAttribute("ips") == null) {
				ips = permissionService.getIpList();
	...
	private boolean permiss(HttpServletRequest request,
		...
		if (user != null) {
			...
			if (request.getSession().getAttribute("menus") == null) {
				PageInfo<NewMenu> pageInfo = newMenuService.getAllNewMenu(1, 0,
						new NewMenu());
			...
			for (NewMenu newMenu : newMenus) {
				if (url.equals(newMenu.getUrl())) {
					...
					if (request.getSession().getAttribute("menu") == null) {
						menus = newMenuService.getNewMenuByUserId(user
								.getUserId());
