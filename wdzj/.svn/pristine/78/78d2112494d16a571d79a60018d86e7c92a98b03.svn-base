package com.duanrong.business.push.service.impl;
...
public class PushServiceImpl implements PushService {
	...
	public Integer pushInvestPerson() {
		...
		if(pushCode == 200){
			if(pushList.size() > 0){
				savePushInvestor(pushList);   //推送记录保存到数据库
			}
			if(p2pEyeUsers.size() > 0){
				savePushTransaction(p2pEyeUsers); //推送交易记录保存到数据库
			}
		}
    ...
    */
	...
	public JSONObject getInvestInfo(PushInfoRow push){
		...
		json.put("order_id",AESUtil.encode(push.getOrder_id()));
